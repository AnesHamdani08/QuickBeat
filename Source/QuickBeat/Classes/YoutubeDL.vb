...
#Region "WPF Support"
        ...
        Public ReadOnly Property QuickLoadCommand As WPF.QuickLoadCommand
            ...
            End Get
        End Property
        <NonSerialized> Private _SearchQuickLoadCommand As New WPF.SearchQuickLoadCommand(Me)
        Public ReadOnly Property SearchQuickLoadCommand As WPF.SearchQuickLoadCommand
            Get
                Return _SearchQuickLoadCommand
            End Get
        End Property
...
#End Region
        ...
        Public Sub New(YTDLPath As String)
            If IO.File.Exists(YTDLPath) Then
                ...
                Configuration.SetStatus("All Good", 100)
            Else
                Configuration.SetStatus("Please provide a valid path.", 0)
        ...
        End Function
        ''' <summary>
        ''' A quick function opposed to <see cref="DumpAndManageVideo(String, Boolean)"/>, gets only essential info:<see cref="YoutubeVideo.Title"/> and <see cref="YoutubeVideo.DirectURL"/>
        ''' </summary>
        ...
        Public Async Function GetVideo(URL As String) As Task(Of YoutubeVideo)
            ...
            If Utilities.CheckInternetConnection Then
                IsBusy = True
            ...
            If Limit >= 1 Then
                If Utilities.CheckInternetConnection Then
                    ...
                    Catch ex As Exception
                        IsBusy = False
                        If Utilities.SharedProperties.Instance.IsLogging Then Utilities.DebugMode.Instance.Log(Of Youtube.YoutubeDL)(ex.ToString)
                    End Try
                    Return Nothing
                End If
            End If
        ...
        Public Async Function SearchVideoAndDump(Query As String) As Task(Of YoutubeVideo)
            ...
            End If
        End Function
        Public Async Function DumpAndManagePlaylist(URL As String, limit As Integer, Optional SkipURLCheck As Boolean = False) As Task(Of IEnumerable(Of YoutubeVideo))
            If Not IO.File.Exists(YoutubeDLLocation) Then Return Nothing
            If Utilities.CheckInternetConnection Then
                ...
                IsBusy = True
                SetKnownState(LinkState.ContactingYTDL)
                Dim YoutubeDL As Process = Process.Start(New ProcessStartInfo(YoutubeDLLocation, "-J --playlist-end " & limit & Space(1) & URL) With {.RedirectStandardOutput = True, .UseShellExecute = False, .WindowStyle = ProcessWindowStyle.Hidden, .CreateNoWindow = True})
        ...
        Public Async Function ParseYoutubeJson(StrJson As String) As Task(Of YoutubeVideo)
            Return Await Task.Run(Function()
                                      Try
                                          ...
                                          Dim Genres As String() = If(Genre?.ToString.Split(","), {})
                                          ...
                                          MediaInfo.ReleaseYear = If(ReleaseYear, 0)
